...
# 데이터셋: Naver Sentiment Movie COrpus(http://github.com/e9t/nsmc/)
# >> 네이버영화 리뷰 중 영화당 100개의 리뷰를 모아
# >> 총200,000개의 리뷰(훈련:15만개, 테스트:5만개)로
# >> 이루어져있고, 1~10점까지의 평점중 중립적인 평점(5~8)은 제외하고 1~4점을 긍정,
# >> 9~10점을 부정으로 동일한 비율로 데이터에 포함 시킴
...
#   평점이 긍정(9~10)점, 부정(1~4)점, 5~8점은 제거

import json
import os
import nltk  # Natural language 처리 단어(자연어)
import matplotlib.pyplot as plt
import numpy as np
from matplotlib import font_manager, rc
from pprint import pprint        # pretty print 이쁘다
from konlpy.tag import Okt
from tensorflow.keras import models  # 구글에서 인공지능 만들때 쓰라고만든게 텐서플로 쉽게 써라고 만들어진게 케라스 같이 사용
...
# kolnpy에는 여러 클래스가 존재하지만, 그중 okt(open korean text)를
# 사용하여 간단한 문장분석을 실행한다.
okt = Okt()
# print(okt.pos('이 밤 그날의 반딧불을 당신의 창 가까이 보낼게요'))
# 트레인, 테스트 데이터셋에 형태소 분석을 통해 풍사태깅 작업 진행
def tokenize(doc):
...
# 분석한 데이터의 토큰(문자열 분석을 위한 적은 단위)의 갯수를 확인
...
# vocab().most common 를 이용해서 가장 자주 사용되는 단어 빈도수 확인
...
# 자주 출현하는 단어50개를 matplotlib을 통해 그래프로 그리기
...
# 이제 데이터를 float 으로 형 변환 시켜주면 데이터 전처리 과정을 끝
print('>> 형변환 시작')
...
# 모델 정의 및 학습하기
# Tensorflow - keras
# - keras 는 층을 조합하여 모델을 생성함
# - 차례대로 층을 쌓아 만든 모델을 tf.keras-Sequential 모델

model = models.Sequential()  # 모델 생성

# 뇌를 만듬
# 모델 층을 구성
model.add(layers.Dense(64, activation='relu', input_shape=(5000,)))   # 1층생성
model.add(layers.Dense(64, activation='relu'))  # 2층생성
...
# 모델 훈련
model.fit(x_train, y_train, epochs=10, batch_size=512)   # 에폭스는 학습을10번반복  512쪽 읽어

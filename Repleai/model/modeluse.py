...
db = client['local']
collection = db.get_collection('movie02')
...
def mongo_select_all():
    for one in collection.find({}, {'_id': 0, 'movieNm': 1, 'content': 1, 'score': 1}): # 제목, 내용, 평점
        reply_list.append([one['movieNm'], one['content'], one['score']])  # dict에서 Value 와 Score 의
    return reply_list
...
def predict_pos_neg(review):
    ...
    data = np.extand_dims(np.asarray(tf).astype('float32'), axis =0)
